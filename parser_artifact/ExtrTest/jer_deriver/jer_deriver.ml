...
let rec expr_of_type typ =
  ...
  | [%type: char]  -> [%expr fun (x:char) -> `Int (Char.code x)]
       ...
       (List.map
          (fun { pcd_name = { txt = name; _ }; pcd_args; _ } ->
             ...
             | Pcstr_tuple args ->
               ...
               let vars = List.init nvars (fun i -> Printf.sprintf "x%d" i) in
               let arg_exprs = List.map2
                 (fun v t -> app (expr_of_type t) [evar v]) vars args in
               let arr = List.fold_right
                 (fun e acc -> [%expr [%e e] :: [%e acc]]) arg_exprs [%expr []] in
               Exp.case
                 (pconstr name (List.map pvar vars))
                 [%expr `List [%e arr]]
             | Pcstr_record _ ->
