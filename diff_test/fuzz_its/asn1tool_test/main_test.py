...
asn1_uper = asn1tools.compile_files('/fuzz_its/ESTI_ITS.asn1', codec='uper', cache_dir='/fuzz_its/asn1tool_test/uper_cache')
asn1_jer = asn1tools.compile_files('/fuzz_its/ESTI_ITS.asn1', codec='jer', cache_dir='/fuzz_its/asn1tool_test/jer_cache')

#encoded = b'\x02\x10\x11\x10\x00\x04\x20\x00\x00\x00'
#z = foo.decode('T2', encoded)
#print(foo_jer.encode('T2', z).decode("utf-8", errors='replace'))
...
while afl.loop(5000):
    ...
    # run ocaml code
    ocaml_result = subprocess.run(["/fuzz_its/verified_format_utf8/_build/default/test/test.exe"], input=data, text=False, capture_output=True)
    ...
    try:
        ...
        # the thing is that pycrate will actually
        # decode unknown ENUM or unknwon CHOICE
        # need to make sure whether asn1tools does the same
        if (ocaml_error == 1):
            try:
                pycrate_dict = tuple_to_dict_c(val)
        ...
        #try:
        pycrate_dict = json.loads(asn1_jer.encode('CAM', val).decode("utf-8", errors='replace'))
        #except:
        #pycrate_dict = tuple_to_dict(dl_dcch())
        #pycrate_dict = (json.loads(dl_dcch.to_jer(pycrate_dict)))
