...
// mod rrc;
// mod generate_rrc_messages;
// generated module


// Here I can change which message type I wanted to test, 
...
#[no_mangle]
pub unsafe extern "C" fn uper_rrc_structure(buf_in: *mut c_char, in_len: isize, buf_out: *mut c_char, out_max: isize) -> isize {
    ...
    let Ok(message) = rrc::DL_DCCH_Message::from_entropy::<_, entropic::scheme::DefaultEntropyScheme>(in_iter) else {
...
#[no_mangle]
pub unsafe extern "C" fn uper_rrc_destructure(buf_in: *mut c_char, in_len: isize, buf_out: *mut c_char, out_max: isize) -> isize {
    let in_len: usize = match in_len.try_into() {
        Ok(l) => l,
        Err(_) => return ASNFUZZGEN_ERR_ARGS,
    };
    ...
    let Ok(pdu) = rrc::DL_DCCH_Message::uper_decode(&mut packet) else {
