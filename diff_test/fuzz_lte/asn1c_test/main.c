...
int main () {
  ...
  while (__AFL_LOOP(10000)) {  // Run 10,000 iterations before restarting
    ...
    */
    ...
    dec_r = uper_decode_complete(NULL, &asn_DEF_DL_DCCH_Message, (void **) &t, buffer, buffer_size);
    ...
    */
    ...
    fork_exec_binary("/fuzz_lte/verified_format/_build/default/test/test.exe", buffer, buffer_size, comp_stream, 0);
	...
	//printf("%s", (stream_buffer + 11));

	
	/* Now we see if the comp-stream is True or False */
    
      
	if (strcmp(stream_buffer + 11, comp_stream_buffer) == 0) {
	  comp_val = 0;
	} else {
	  comp_val = 1;
	}
    ...
    //printf("%s", comp_stream_buffer);
    //printf("\n%s", stream_buffer);
    //printf (": Compare Value is %d", comp_val);
